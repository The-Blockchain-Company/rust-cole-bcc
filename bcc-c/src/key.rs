use bcc::hdwallet;
use std::{ptr, slice};
use types::{BccResult, XPrvPtr, XPubPtr};

#[no_mangle]
pub extern "C" fn bcc_xprv_derive(c_xprv: XPrvPtr, index: u32) -> XPrvPtr {
...
#[no_mangle]
pub extern "C" fn bcc_xprv_from_bytes(
    c_xprv: *const u8,
    ...
    match hdwallet::XPrv::from_bytes_verified(array) {
        ...
        Err(_) => BccResult::failure(),
    }
}

#[no_mangle]
pub extern "C" fn bcc_xprv_to_bytes(c_xprv: XPrvPtr) -> *const u8 {
...
#[no_mangle]
pub extern "C" fn bcc_xprv_bytes_delete(bytes: *mut u8) {
    let mut vector =
        unsafe { Vec::from_raw_parts(bytes, hdwallet::XPRV_SIZE, hdwallet::XPRV_SIZE) };
    bcc::util::securemem::zero(&mut vector);
    std::mem::drop(vector)
}

#[no_mangle]
pub extern "C" fn bcc_xprv_to_xpub(c_xprv: XPrvPtr) -> XPubPtr {
    let xprv = unsafe { c_xprv.as_mut() }.expect("Not a NULL PTR");
...
#[no_mangle]
pub extern "C" fn bcc_xprv_delete(c_xprv: XPrvPtr) {
    unsafe { Box::from_raw(c_xprv) };
}

#[no_mangle]
pub extern "C" fn bcc_xpub_derive(c_xpub: XPubPtr, index: u32) -> XPubPtr {
...
#[no_mangle]
pub extern "C" fn bcc_xpub_delete(c_xpub: XPubPtr) {
