...
use bcc::hdwallet;
use bcc::hdwallet::XPrv;
use bcc::wallet::bip44;
use bcc::wallet::scheme::Wallet;

use std::os::raw::c_char;
use std::{ffi, ptr, slice};

use address::ffi_address_to_base58;
use types::{AccountPtr, BccResult, WalletPtr, XPrvPtr};
...
///
/// use the function `bcc_wallet_delete` to free all the memory associated to the returned
...
///
#[no_mangle]
pub extern "C" fn bcc_wallet_new(
...
///
/// The data must be a valid Wallet created by `bcc_wallet_new`.
#[no_mangle]
pub extern "C" fn bcc_wallet_delete(wallet_ptr: WalletPtr) {
    unsafe { Box::from_raw(wallet_ptr) };
}

#[no_mangle]
pub extern "C" fn bcc_wallet_root_key(wallet_ptr: WalletPtr) -> XPrvPtr {
...
/// To avoid memory leak, use `bcc_account_delete`
///
#[no_mangle]
pub extern "C" fn bcc_account_create(
...
#[no_mangle]
pub extern "C" fn bcc_account_delete(account_ptr: AccountPtr) {
    unsafe { Box::from_raw(account_ptr) };
}

#[no_mangle]
pub extern "C" fn bcc_account_generate_addresses(
...
#[no_mangle]
pub extern "C" fn bcc_account_delete_addresses(addresses_ptr: *mut *mut c_char, size: usize) {
    for i in 0..size {
        unsafe {
