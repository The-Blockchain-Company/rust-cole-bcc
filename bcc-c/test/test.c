...
void test_can_create_address(void)
{
    ...
    bcc_account *account = bcc_account_create(wallet, alias, 0);

    TEST_ASSERT_MESSAGE(account, "The account creation failed");

    bcc_account_generate_addresses(account, 0, 0, NUMBER_OF_ADDRESSES, address, PROTOCOL_MAGIC);

    TEST_ASSERT_MESSAGE(!bcc_address_is_valid(address[0]), "The generated address is invalid");
