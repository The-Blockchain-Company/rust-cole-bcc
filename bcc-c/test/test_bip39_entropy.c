...
void test_generate_entropy_from_mnemonics(void) {
    ...
    bcc_entropy entropy;
    uint32_t bytes;
    bcc_bip39_error_t error = bcc_entropy_from_english_mnemonics(mnemonics, &entropy, &bytes);
...
void test_generate_entropy_from_random_generator(void) {
    ...
    bcc_entropy entropy;
    uint32_t bytes;
    bcc_bip39_error_t error = bcc_entropy_from_random(NUMBER_OF_WORDS, gen, &entropy, &bytes);
