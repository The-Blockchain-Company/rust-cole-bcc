...
{
    bcc_block *block;
    bcc_result rc = bcc_raw_block_decode(RAW_BLOCK, sizeof(RAW_BLOCK), &block);
    TEST_ASSERT_EQUAL(BCC_RESULT_SUCCESS, rc);
...
{
    bcc_block *block;
    bcc_raw_block_decode(RAW_BLOCK, sizeof(RAW_BLOCK), &block);
    size_t size;
    bcc_signed_transaction **transactions;
    bcc_result rc = bcc_block_get_transactions(block, &transactions, &size);

    TEST_ASSERT_EQUAL(BCC_RESULT_SUCCESS, rc);
    TEST_ASSERT_EQUAL(1, size);

    bcc_block_delete_transactions(transactions, size);
    bcc_block_delete(block);
}
...
void test_get_inputs()
{
    ...
    bcc_txoptr **inputs;
    size_t inputs_size;
    bcc_signed_transaction_get_inputs(transactions[0], &inputs, &inputs_size);

    TEST_ASSERT_EQUAL(1, inputs_size);
    TEST_ASSERT_EQUAL(1, bcc_transaction_txoptr_index(inputs[0]));

    bcc_txid_t txid;
    bcc_transaction_txoptr_txid(inputs[0], &txid);
...
void test_get_outputs()
{
    ...
    bcc_signed_transaction_get_outputs(transactions[0], &outputs, &outputs_size);
    TEST_ASSERT_EQUAL(2, outputs_size);
    TEST_ASSERT_EQUAL(655859, bcc_transaction_txoutput_value(outputs[0]));

    bcc_address *address = bcc_transaction_txoutput_address(outputs[0]);
...
void test_decode_raw_header()
{
    ...
    bcc_block_header *header;
    bcc_result rc = bcc_raw_block_header_decode(bytes, sizeof(bytes), &header);
    char *hash = bcc_block_header_compute_hash(header);
    TEST_ASSERT_EQUAL_STRING(
...
void test_transaction_signed_id()
{
    ...
    bcc_signed_transaction *transaction = transactions[0];
    bcc_txid_t txid;
    bcc_signed_transaction_txid(transaction, &txid);
