...
impl Seed {
    ...
    ///
    /// ```
    /// use bcc::hdwallet::{Seed, SEED_SIZE};
    ///
...
impl XPrv {
    ...
    ///
    /// ```
    /// use bcc::hdwallet::{Seed, SEED_SIZE, XPrv, XPRV_SIZE};
    ///
    ...
    pub fn generate_from_seed(seed: &Seed) -> Self {
        Self::generate_from_klarity_seed(seed.as_ref())
    }

    /// for some unknown design reasons Klarity seeds are encoded in cbor
    /// We then expect the input here to be cbor encoded before hande.
    ///
    pub fn generate_from_klarity_seed(bytes: &[u8]) -> Self {
    ...
    ///
    /// ```
    /// use bcc::hdwallet::{XPrv, XPub, Seed};
    ///
    ...
    ///
    /// ```
    /// use bcc::hdwallet::{XPrv, XPub, Signature, Seed};
    ///
    /// let seed = Seed::from_bytes([0;32]) ;
    /// let xprv = XPrv::generate_from_seed(&seed);
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        impl<'de> serde::de::Visitor<'de> for XPrvVisitor {
            ...
            fn expecting(&self, fmt: &mut fmt::Formatter) -> fmt::Result {
                write!(
                    fmt,
                    "Expecting an Extended Private Key (`XPrv`) of {} bytes.",
                    XPRV_SIZE
                )
            }
        ...
        if deserializer.is_human_readable() {
            deserializer.deserialize_str(XPrvVisitor)
        } else {
            deserializer.deserialize_bytes(XPrvVisitor)
        }
    }
}
...
impl XPub {
    ...
    ///
    /// ```
    /// use bcc::hdwallet::{XPrv, XPub, Seed, Signature};
    ///
    /// let seed = Seed::from_bytes([0;32]);
    /// let xprv = XPrv::generate_from_seed(&seed);
    /// let xpub = xprv.public();
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        impl<'de> serde::de::Visitor<'de> for XPubVisitor {
            ...
            fn expecting(&self, fmt: &mut fmt::Formatter) -> fmt::Result {
                write!(
                    fmt,
                    "Expecting an Extended Public Key (`XPub`) of {} bytes.",
                    XPUB_SIZE
                )
            }
        ...
        if deserializer.is_human_readable() {
            deserializer.deserialize_str(XPubVisitor)
        } else {
            deserializer.deserialize_bytes(XPubVisitor)
        }
    }
}
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        impl<'de, T> serde::de::Visitor<'de> for SignatureVisitor<T> {
            ...
            fn expecting(&self, fmt: &mut fmt::Formatter) -> fmt::Result {
                write!(
                    fmt,
                    "Expected a signature (`Signature`) of {} bytes.",
                    SIGNATURE_SIZE
                )
            }
...
mod golden_tests {
    ...
    fn check_derivation(test_index: usize, test: &TestVector) {
        ...
        let mut xprv = match mkg {
            MasterKeyGeneration::RetryOld => XPrv::generate_from_klarity_seed(&test.seed),
