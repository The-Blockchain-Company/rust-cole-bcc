...
mod unit_tests {
    ...
    fn mk_random_klarity_style_address() -> ExtendedAddr {
    ...
    fn mk_klarity_style_input(value: Coin) -> Input<()> {
        let txid = TxId::new(&vec![random(), random()]);
        let txoptr = TxoPointer::new(txid, random());
        let address = mk_random_klarity_style_address();
    ...
    fn mk_klarity_style_txout(value: Coin) -> TxOut {
        let address = mk_random_klarity_style_address();
    ...
    {
        let change_address = mk_random_klarity_style_address();
    ...
    fn random_small_amount_ada_first_match_first() {
        let input1 = mk_klarity_style_input(Coin::new(1).unwrap());
        let input2 = mk_klarity_style_input(Coin::new(2).unwrap());
        let input3 = mk_icarus_style_input(Coin::new(3).unwrap());
        let input4 = mk_klarity_style_input(Coin::new(1_000_000).unwrap());
        let output1 = mk_icarus_style_txout(Coin::new(1).unwrap());
        let output2 = mk_klarity_style_txout(Coin::new(2).unwrap());
