use base64;
use bcc::{address, block, coin, config, fee, hdwallet, redeem};
use serde_json;
...
pub fn parse<R: Read>(json: R) -> config::GenesisData {
    ...
    for (stakeholder_id, weight) in &data.bootStakeholders {
        ...
        let psk = bcc::block::sign::ProxySecretKey {
            omega: 0,
...
mod test {
    ...
    use super::*;
    use bcc::{coin, fee::Milli};

    #[test]
    pub fn test() {
        let genesis_hash = bcc::block::HeaderHash::from_str(
