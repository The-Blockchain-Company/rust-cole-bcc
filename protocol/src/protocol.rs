...
pub struct Connection<T> {
    ...
    latest_tip: Option<bcc::block::BlockHeader>,
}
...
impl<T: Write + Read> Connection<T> {
    ...
    pub fn process_async_headers(&mut self, msg: &[u8]) -> Result<()> {
        ...
        let mut headers = bcc::block::BlockHeaders::deserialize(&mut de)?;
    ...
    pub fn get_latest_tip(&self) -> Option<bcc::block::BlockHeader> {
...
pub mod command {
    ...
    pub fn stream_blocks<W: Read + Write, F>(
        ...
        from: &[bcc::block::HeaderHash],
        to: bcc::block::HeaderHash,
        got_block: &mut F,
    ) -> Result<()>
    where
        F: FnMut(bcc::block::RawBlock) -> Result<()>,
    {
        ...
        loop {
            ...
            match bcc::cbor::hs::util::decode_sum_type(&mut msg)? {
                0 => {
                    ...
                    let rblk = bcc::block::RawBlock::from_dat(raw_block);
                    got_block(rblk)?;
                }
                1 => {
    ...
    pub struct GetBlockHeader {
        from: Vec<bcc::block::HeaderHash>,
        to: Option<bcc::block::HeaderHash>,
    }
    impl GetBlockHeader {
        pub fn tip() -> Self {
            GetBlockHeader {
        ...
        pub fn range(from: &[bcc::block::HeaderHash], to: bcc::block::HeaderHash) -> Self {
            GetBlockHeader {
    ...
    where
        W: Read + Write,
    {
        type Output = bcc::block::RawBlockHeaderMultiple;
        fn command(&self, connection: &mut Connection<W>, id: LightId) -> Result<()> {
        ...
        fn result(&self, connection: &mut Connection<W>, id: LightId) -> Result<Self::Output> {
            ...
            match decode_sum_type(&dat) {
                ...
                Some((0, dat)) => {
                    ...
                    Ok(bcc::block::RawBlockHeaderMultiple::from_dat(v))
                }
                Some((1, dat)) => {
    ...
    impl GetBlock {
        pub fn only(hh: &bcc::block::HeaderHash) -> Self {
            GetBlock::from(&hh.clone(), &hh.clone())
        }
        pub fn from(from: &bcc::block::HeaderHash, to: &bcc::block::HeaderHash) -> Self {
            GetBlock {
                from: from.clone(),
                to: to.clone(),
            }
    ...
    fn strip_msg_response(msg: &[u8]) -> Result<bcc::block::RawBlock> {
        ...
        match decode_sum_type(msg) {
            ...
            Some((0, dat)) => {
                let mut v = Vec::new();
                v.extend_from_slice(dat);
                Ok(bcc::block::RawBlock::from_dat(v))
            }
            Some((1, dat)) => {
                let mut msg = Deserializer::from(Cursor::new(&dat));
                Err(Error::ServerError(msg.text()?))
    ...
    where
        W: Read + Write,
    {
        type Output = Vec<bcc::block::RawBlock>;
        fn command(&self, connection: &mut Connection<W>, id: LightId) -> Result<()> {
            // require the initial header
