...
use bcc::block::{Block, BlockDate, ChainState, EpochId, EpochSlotId, HeaderHash, Utxos};
use bcc::config::GenesisData;
use bcc::tx::TxoPointer;
...
pub fn write_chain_state_delta<W: Write>(
    ...
    let (removed_utxos, added_utxos) =
        bcc::util::diff_maps::diff_maps(&parent_chain_state.utxos, &chain_state.utxos);
